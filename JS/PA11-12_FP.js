// Task 1: Build a function-based console log message generator(now using += operators-->DID NOT WORK! had to remove.)
function consoleStyler(color, background, fontSize, txt) {
    var message = "%c" + txt;
    var style = `color: ${color}; background: ${background}; font-size: ${fontSize};`;
    console.log(message, style);
    console.log(message, color, background, fontSize, txt);
}

// Task 2: Build another console log message generator
function celebrateStyler(reason) {
   var fontsStyle = "color: tomato; font-size: 50px";
    if (reason == "birthday") {
        console.log(`%cHappy birthday`, fontsStyle);
    } else if (reason == "champions") {
        console.log(`%cCongrats on the title!`, fontsStyle);
    } else {
        console.log(message, style);
    }
}
// replaced single quotes with back-ticks in arguments
// Task 3: Run both the consoleStyler and the celebrateStyler functions
consoleStyler(`#1d5c63`, `#ede6db`, `40px`, `Congrats!`);
celebrateStyler(`birthday`);

// Task 4: Insert a congratulatory and custom message
/* function styleAndCelebrate() {
    consoleStyler('ef7c8e', 'fae8e0', '30px', 'You made it!');
    celebrateStyler('champions');
    
} */
function styleAndCelebrate(color, background, fontSize, txt, reason) {
    consoleStyler(color, background, fontSize, txt);
    celebrateStyler(reason)
}
// Call styleAndCelebrate
styleAndCelebrate(`ef7c8e`, `fae8e0`, `30px`, `You made it!`, `champions`);

/* Failed Test 1: Not logging the consoleStyler() variables
   Passed Test 2: Successfully logged celebrateStyler() variables
   Failed Test 3: Not calling consoleStyler() and celebrateStyler()
   Failed Test 4: Not calling styleAndCelebrate() */